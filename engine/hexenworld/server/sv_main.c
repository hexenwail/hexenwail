...
/* The shared progs code (h2shared/pr_edict.c) allocates edicts against a
 * RUNTIME limit now, not the compile-time MAX_EDICTS it used to -- Hexen II
 * grew a max_edicts cvar and ED_Alloc follows it.  HexenWorld has no such
 * cvar, so give it the same variable pinned to its own ceiling; that keeps
 * the shared file free of #ifdefs and leaves the obvious place to hang a
 * cvar on if HexenWorld ever wants one. */
...
void SV_DropClient (client_t *drop)
{
	...
	if (drop->state == cs_spawned)
	{
		...
		else if (SpectatorDisconnect)
		{
			...
			// this will set the body to a dead frame, among other things
			*sv_globals.self = EDICT_TO_PROG(drop->edict);
			PR_ExecuteProgram (SpectatorDisconnect, "SpectatorDisconnect");
		}
	}
...
static void SVC_DirectConnect (void)
{
	...
	*newcl = temp;
	...
	PR_ExecuteProgram (*sv_globals.SetNewParms, "SetNewParms");
