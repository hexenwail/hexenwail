...
/* Hexenwail local edit: Ironwail pulls in SDL here purely to reach SDL_BYTEORDER
   for the MINIZ_LITTLE_ENDIAN define below.  quakefs.c is shared code that is
   also compiled into the SDL-free dedicated server, so the include cannot
   stand.  Removing both lets miniz's own detection further down (the
   "Set MINIZ_LITTLE_ENDIAN only if not set" block) do the job -- it reads
   __BYTE_ORDER__ on GCC/Clang and falls back to an x86 check otherwise. */
...
/* MINIZ_LITTLE_ENDIAN: see the note above -- left to miniz's own detection. */
...
/* Hexenwail local edit: Ironwail dropped these two declarations and disabled
   their definitions in miniz.c behind an "unused for now" #if 0, which we have
   re-enabled.  We need extract_to_mem so a zip entry can be inflated straight
   into a caller-supplied hunk/zone buffer instead of going through an extra
   full-size heap allocation and copy. */
