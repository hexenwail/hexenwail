...
/* Sidecar .vis, the third and last of this engine's brush loaders to get it
 * (gl_model.c under uhexen2-a5nn.8, sv_model.c after it, this one under
 * uhexen2-a5nn.21).  This file is the software renderer's loader and compiles
 * only for the emscripten WEB_RENDERER=software target, so the drift was easy
 * to miss: `scripts/wasm-build.sh software` is the only build that reaches it. */
 ...
 * in a malloc'd buffer rather than as a lump inside mod_base, can run the same
 * conversion.  Mirrors the same split in gl_model.c.  uhexen2-a5nn.21. */
...
/*
=================
External .vis support (uhexen2-a5nn.21)

A vispatch file is a concatenation of records, each

    char mapname[32]; int filelen;
...
header, which is how the search below skips to the next record.
...
Deliberately a straight port of gl_model.c's block rather than a shared one:
the three loaders keep their own copies of this and of external_ents, and
unifying them is a bigger change than closing the drift.
=================
*/
...
static FILE *Mod_FindVisibilityExternal (void)
{
	...
	/* A .vis from a lower-priority gamedir must not override the map it was
	 * not shipped with -- the same path_id rule external_ents uses. */
	if (path_id < loadmodel->path_id)
	{
		fclose (f);
		Con_DPrintf ("ignored %s from a gamedir with lower priority\n", visfilename);
...
// load into heap
	...
	/* Try the sidecar .vis BEFORE the embedded lumps rather than loading them
	 * and overwriting: the hunk has no free-in-the-middle, so loading first
	 * would strand the embedded copies for the map's lifetime.
	 *
	 ...
	 * Same block as gl_model.c's.  uhexen2-a5nn.21. */
	{
