...
/* Stub implementations for file I/O functions not present in uhexen2 */
static FILE *open_files[16];
static int open_file_count = 0;

int FS_FileOpenWrite(const char *path)
{
	if (open_file_count >= 16)
		return -1;
	open_files[open_file_count] = fopen(path, "wb");
	if (!open_files[open_file_count])
		return -1;
...
{
	if (handle < 0 || handle >= open_file_count || !open_files[handle])
		return 0;
	return fwrite(data, 1, count, open_files[handle]);
}
