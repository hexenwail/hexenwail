...
static const char cull_mark_src[] =
	...
	"    if (!planeback && d < -0.01) return; */\n"
	"\n"
	"    /* Padded frustum cull — reject surfaces clearly behind/beside\n"
	"     * the camera to prevent near-plane clip artifacts (which cause\n"
	"     * phantom depth writes that occlude water).  A generous slack\n"
	"     * of 256 units avoids the tight-frustum surface popping. */\n"
	"    const float FRUSTUM_SLACK = 256.0;\n"
	"    for (int fi = 0; fi < 4; fi++) {\n"
	"        vec3 n = u_frustum[fi].xyz;\n"
	"        float d = u_frustum[fi].w;\n"
	"        /* Test AABB against plane: pick the corner most in the\n"
	"         * direction of the plane normal (p-vertex). */\n"
	"        vec3 pv = vec3(\n"
	"            n.x >= 0.0 ? s.maxs.x : s.mins.x,\n"
	"            n.y >= 0.0 ? s.maxs.y : s.mins.y,\n"
	"            n.z >= 0.0 ? s.maxs.z : s.mins.z);\n"
	"        if (dot(n, pv) + d < -FRUSTUM_SLACK) return;\n"
	"    }\n"
	"\n"
