...
# Build sizebuf.c with renamed exports and compare it with the consolidated Rust
# engine staticlib's sizebuf feature.
set -euo pipefail
...
if [ ! -f "$RUST_LIB" ]; then
	...
	echo "       build it first: cargo build --release --offline --manifest-path $rust_root/Cargo.toml --features hashindex,mathlib,sizebuf" >&2
	exit 2
fi
...
RENAME=(
	-DSZ_Init=c_SZ_Init
	-DSZ_Clear=c_SZ_Clear
	-DSZ_GetSpace=c_SZ_GetSpace
	-DSZ_Write=c_SZ_Write
	-DSZ_Print=c_SZ_Print
)

echo "== compiling the C original with renamed symbols =="
cc "${INCS[@]}" "${CFLAGS[@]}" -DGLQUAKE "${RENAME[@]}" \
	-c "$engine/h2shared/sizebuf.c" -o "$OUT/sizebuf_c.o"
...
cc -o "$OUT/diff_harness" "$OUT/diff_harness.o" "$OUT/sizebuf_c.o" "$RUST_LIB" -lm
echo "== running =="
"$OUT/diff_harness"
echo "RESULT: PASS"
