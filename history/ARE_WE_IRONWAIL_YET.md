...
**One open Ironwail port has no row in the tables above**, because it is upstream work rather than a partial feature: uhexen2-ayrn (light-trace cache for alias models, Ironwail `e2f39505`).  It is implemented and on master (`rlight: cache the alias light trace`), builds clean, and passed a static audit on 2026-08-20; what it still lacks is runtime verification — an in-game look with `r_shadows 1` for correct shadow floor height, and a flickering lightstyle still animating on a stationary model.  The other two listed here until 2026-08-20 have closed: uhexen2-r7zu (`TEXPREF_UNCOMPRESSED` for alpha-tested and 2D textures) and uhexen2-yae (menu system improvements).  These are counted in the umbrella epic uhexen2-a5nn, not in the parity percentage; that epic now closes when ayrn's visual check passes.  Run `bd show uhexen2-a5nn` for the current child list.
...
### P1 — High
1. **Light-trace cache for alias models** — Ironwail `e2f39505` (uhexen2-ayrn).  Implemented, on master, building clean, statically audited 2026-08-20 with no defects found.  Gated solely on a visual check: `r_shadows 1` with shadows at the right floor height, and a torch lightstyle still animating on a stationary model.  This is the last item holding the umbrella epic uhexen2-a5nn open.
