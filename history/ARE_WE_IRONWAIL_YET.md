...
**Parity: ~99% ported, 0% partial, 1% missing** (excluding N/A).  Recomputed from the actual per-row symbols in the tables below.  Both former 🔶 closed 2026-08-20: MD5mesh/IQM skeletal support was completed, and the frame-major alias pose SSBO row turned out to already be satisfied.  The single ❌ is bindless textures, which moved out of ➖ N/A on 2026-08-18 after Ironwail was checked rather than assumed.
...
As of 2026-08-20, **no** feature in the tables above is partial.  MD5mesh/IQM was completed (uhexen2-7ok0) and Alias model GPU data layout was found already satisfied (uhexen2-48fx) — see both rows.  Pointfile leak visualization completed 2026-08-14 (uhexen2-m183, `c293ae22a`).  Bindless textures remain ❌ Missing (uhexen2-im9g), reclassified out of ➖ N/A on 2026-08-18 once Ironwail was actually checked: it has a working bindless path and this is a real gap.  The scaffolding removal in `bbf4a97a5` (uhexen2-ubsu) stands — that code could never run.  A `c_texbinds` counter was added to `r_speeds 2` on 2026-08-20 so the bead's own "measure first" gate can finally be answered.  MSAA with FBO resolve is ➖ — Hexenwail intentionally uses FXAA, vestigial window MSAA dropped in `5b6d57ec4`.  Scorecard: 87 ✅ / 0 🔶 / 1 ❌ / 10 ➖ (~99% parity, excluding N/A).
