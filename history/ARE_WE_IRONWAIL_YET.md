...
Last updated: 2026-08-22 (**The physics tick was running at 20 Hz, and the field caught it before we did.**  The Decoupled-renderer row said "physics at `sys_ticrate` (20 Hz)" and was accurate; what nobody rechecked was whether 20 Hz was the *right* number.  It was not — upstream uHexen2 ticked the world once per render frame up to `host_maxfps` = 72, and Raven's own disabled `FPS_20` arm in the same file substeps at `1.0/72.0`.  BloodShot reported the symptoms on Discord (2026-08-21) with side-by-side video: slower spider attacks, mod think chains off-speed, strafe jumping bleeding speed.  Fixed in uhexen2-skjv — the tick moved to its own archived cvar `sv_physfps`, default 72, and `sys_ticrate` went back to meaning the dedicated-server interval.  The lesson for this document is narrow and worth keeping: a row can cite the code correctly and still record a defect, because "what the engine does" and "what the engine should do" are different claims and only the first one is greppable.  Field confirmation is tracked as uhexen2-fkdx.  Scorecard unchanged at 87 / 0 / 1 / 9 — this was a regression against upstream Hexen II, not an Ironwail parity row.  **The bindless ❌ picked up independent corroboration** from Spoike on the same Discord thread: no WebGL2 equivalent, so the path would fork our world draw against the `#version 300 es` build.  Recorded in the row.  Prior entry (2026-08-20): **Parity epic closed: the last open Ironwail port passed its runtime check.**  uhexen2-ayrn (light-trace cache, `e2f39505`) was implemented, on master and statically audited, waiting only on a visual confirmation; that ran headless under Xvfb and passed on both claims — shadow on the floor plane at the boot soles deep into a cache-hit burst, and a stationary model's shading tracking an animated lightstyle at r=+0.9953 against the world lightmap, 34.5% swing, HUD pixel-identical.  With ayrn closed, umbrella epic uhexen2-a5nn closed too — its own note named that check as its gate.  **The verification passed vacuously three times first**, and the trap is recorded in Bead Coverage: demo1 has zero floor faces with an animated style, and the BSP scan used to find one read `dface_t.styles[]` two bytes off, inventing 905 animated faces in a map that has none.  `tools/bsp_lightstyles.py` now answers that offline.  **Upstream baseline moved to `7ff4ec57`** — the one Ironwail commit past `4fa8bfbd` (unsigned mask constants in `gl_shaders.h`) was triaged not-applicable as uhexen2-7g1x: our shaders already write all three masks the fixed way, because the `#version 300 es` WebGL2 path makes a mixed-sign mask a build error rather than merely untidy.  Scorecard unchanged at 87 / 0 / 1 / 9.  Prior entry (2026-08-20): **Bindless textures ❌ decided, and the scorecard recounted mechanically.**  The table had drifted to 85 ✅ / 2 🔶 / 1 ❌ / 9 ➖: the 2026-08-20 promotion of MD5mesh/IQM and the alias pose-layout row edited both row symbols and the parity sentence but not the counts that summarise them.  Recounted from the actual row symbols — **87 / 0 / 1 / 9**.  The ➖ figure was separately written as 10 in the Bead Coverage paragraph and in uhexen2-a5nn; the rows say 9.  **Bindless textures stays ❌ but is now a decision rather than a backlog item** — uhexen2-im9g closed, its measure-first gate answered from map content instead of a profiler: `DrawTextureChains` binds once per texture chain, so the distinct textures a map's faces reference cap the world pass at 119 across all 59 shipped maps, mean 58.  It stays ❌ and not ➖ because a gap declined on measured grounds is still a gap.  **Two dead commit hashes fixed** — the bindless row and the shortlist both cited `8124ed055` for the scaffolding removal, which an interactive rebase replaced with `bbf4a97a5`; the Bead Coverage paragraph already had it right.  Today's own work is cited by tool path and bead ID rather than hash, because hashes in this repo have been rewritten twice in a day.  **Bead Coverage's list of open Ironwail ports cut from three to one** — uhexen2-r7zu and uhexen2-yae closed; only uhexen2-ayrn remains, implemented and audited but awaiting a visual check, and it is now what holds uhexen2-a5nn open.  Prior entry (2026-08-18): **Documentation correctness pass**, prompted by a reader spotting that the Steam row justified ➖ with "Hexen II not on Steam" — Hexen II *is* sold on Steam; the real reason is that the engine needs no Steam-specific code.  Checking that one claim turned up five more, all of the same kind: statements about Ironwail written down without being checked against Ironwail.  **Bindless textures ➖ N/A → ❌ Missing** — the row asserted in three places that "Ironwail does not use `ARB_bindless_texture` either", which is false; Ironwail's path is live, extension-gated in `gl_vidsdl.c`, and used in `r_world.c`.  **Three features were wrongly demoted out of Exclusives** — `gl_waterripple` and the `gl_glows`/`gl_other_glows`/`gl_missile_glows` trio do not exist in Ironwail at all; their supporting citations (`gl_rmain.c:127-129`, `:133`, `:134`) resolve to `map_wateralpha`, `r_scale` and a blank line, i.e. they came from grepping the wrong tree.  `r_texture_external` stays demoted but with a corrected reason.  **The HD replacement packs exclusive added earlier the same day was overstated** — Ironwail already has per-map texture directories and `_glow`/`_luma` sidecars; only the DDS/KTX container path is exclusive.  **MAX_ENT_LEAFS** is 64 here, not 32 — we passed Ironwail in `32c35acdb`.  **Every `file.c:NNN` citation in the document had drifted** and has been converted to file + symbol; see the note above the porting workflow.  Verified-correct claims left alone: `sys_ticrate` 0.05 = 20 Hz, MAX_VISEDICTS 16384 = Ironwail, FRAMES_IN_FLIGHT 3, `msaa_steps[]`, and the `snd_waterfx` / zoom / `cl_gun_fovscale` demotions.  Prior entry (2026-08-18, earlier): audit against Ironwail `4fa8bfbd`; upstream queue empty; pointfile 🔶→✅; `r_alias_stochastic_alpha` dropped; MD5 row rewritten; caustics moved to Exclusives; four bug-fix ports and a tab-completion row added.  Prior entry (2026-08-13): bindlist.lst ✅ and pointfile 🔶 added; scorecard recomputed.)
...
| Category | ✅ | 🔶 | ❌ | ➖ |
|---|---|---|---|---|
| Rendering — GPU Pipeline | 13 | 0 | 1 | 0 |
...
**Parity: ~99% ported (87/88), 0% partial, 1% missing** (excluding N/A).  Recomputed from the actual per-row symbols in the tables below — mechanically, not by hand; the table above had drifted to 85/2/1/9 because the 2026-08-20 promotion edited this sentence and both row symbols but not the counts they summarise.  Both former 🔶 closed 2026-08-20: MD5mesh/IQM skeletal support was completed, and the frame-major alias pose SSBO row turned out to already be satisfied.

The single ❌ is bindless textures, and it is now a **decision, not a backlog item**: reclassified out of ➖ N/A on 2026-08-18 once Ironwail was actually checked, then declined on measured grounds on 2026-08-20 (uhexen2-im9g closed).  It stays ❌ rather than becoming ➖ precisely because it is a real gap we chose not to close — see the row for the measurement.
...
### P1 — High
*Empty.  No Ironwail parity item is outstanding; the only ❌ row, bindless textures, is a recorded decision rather than queued work.*
